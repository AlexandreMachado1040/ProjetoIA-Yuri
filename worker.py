...
_DEFAULTS = dict(
    ...
    tipo_montagem="livre",
    degradacao_anual_pct=0.5, ano_operacao=1,
    ...
    if method == "POST" and path in ("/simulate", "/daily", "/"):
        ...
        try:
            ...
            params = {
                ...
                "perda_mismatch_pct": max(0.0, min(float(p.get("perda_mismatch_pct", 2.0)), 10.0)),
                "perda_lid_pct": max(0.0, min(float(p.get("perda_lid_pct", 0.2)), 5.0)),
                "ganho_qualidade_pct": max(-3.0, min(float(p.get("ganho_qualidade_pct", 0.4)), 3.0)),
                "indisponibilidade_pct": max(0.0, min(float(p.get("indisponibilidade_pct", 0.0)), 20.0)),
                "degradacao_anual_pct": max(0.0, min(float(p.get("degradacao_anual_pct", 0.5)), 5.0)),
                "ano_operacao": max(1, min(int(p.get("ano_operacao", 1)), 40)),
                "vida_util_anos": max(1, min(int(p.get("vida_util_anos", 25)), 40)),
                "modulo":     mod,
                "inversor":   inv,
